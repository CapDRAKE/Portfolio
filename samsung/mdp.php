<html>
	...
	<body>
		...
		<div id = "contenu-page">
			...
			<!--Fin EnTete -->
			<div class="compte">
				<center>Modifier le mot de passe d'un utilisateur</center>
			</div>
			...
			<!--PHP de modification du mdp-->
				<?php
						...
						if(isset($_POST['mdp'])&&isset($_POST['pseudo'])){
							$pseudo = $_POST['pseudo'];
							$mdp =$_POST['mdp'];
							$insertion = mysqli_query($bdd, "UPDATE utilisateur SET mdpUtilisateur =sha1('$mdp') WHERE identifiant = '".$pseudo."';");
							echo "<script>alert(\"Mot de passe bien modifié\")</script>";
						}
						else{
							echo "<script>alert(\"Merci de saisir l'identifiant et le nouveau mot de passe de l'utilisateur\")</script>";
						}
					?>
